		...
		Prawn::Document.generate("#{dir_factures_date}/facture_#{facture.numfacture}.pdf") do
			...
			if facture.accompte == 100
				tva = [[Prawn::Table::Cell.new(
					...
					[Prawn::Table::Cell.new(
						:text=> "TTC", :document => self, :font_size=>10, :font_style => :bold), Prawn::Table::Cell.new(:text => format("%.2f", (totalsumgeneral * 0.196) + totalsumgeneral).to_s, :document => self, :font_style => :bold, :font_size=>10)
				]]
			else
